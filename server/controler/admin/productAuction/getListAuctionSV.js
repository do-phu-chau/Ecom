...
const ProductService = {
  getListProductAuctionService: (page = 1, limit = 5, search) => new Promise(async (resolve, reject) => {
    try {
      ...
      const products = await ProductAuction.aggregate([
        matchStage,
        {
          $lookup: {
            from: 'categories', 
      ...
      resolve({
        success: true,
        err: 0,
        msg: products.length ? 'OK' : 'Không tìm thấy sản phẩm nào.',
        status: 200,
        response: {
          total,
          products,
      ...
      reject({
        success: false,
        err: 1,
        msg: 'không có sản phẩm: ' + (error.message || 'Đã xảy ra lỗi không xác định'),
        status: 500,
      });
    }
  }),
