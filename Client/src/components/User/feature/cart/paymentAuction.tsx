...
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { createOrdeAuctionThunk } from "../../../../redux/order/orderThunks";
import { fetchCartById } from "../../../../redux/cart/cartThunk";
import { AppDispatch, RootState } from "../../../../redux/store";
import { Order } from "../../../../types/order/order";
import { useForm, SubmitHandler } from "react-hook-form";
import { useVNPayAuction } from "../../../../hooks/vnpay";
import { itemAuction } from "../../../../types/cart/carts";
import { Card } from "flowbite-react";
...
const CheckoutPage: React.FC = () => {
      ...
      cartDetails: [
        {
          ...
          items: cart?.itemAuction.map((item: itemAuction) => ({
            ...
            //   product_attributes: Array.isArray(item.auctionPricingRange.product_randBib.product_attributes)
  ...
  const handleOrderCreation = async (orderId: string) => {
    ...
    try {
      const response = await dispatch(
        createOrdeAuctionThunk(orderData)
      ).unwrap();
      toast.dismiss();
  ...
  const handleCheckout: SubmitHandler<FormData> = async () => {
    ...
    if (!cart || cart.itemAuction.length === 0) {
      toast.error("Giỏ hàng trống.");
      return;
    }
        ...
        <form onSubmit={handleSubmit(handleCheckout)}>
          ...
          <div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
            <div className="px-4 pt-8 min-h-[calc(85vh-10rem)]">
              ...
              {cart &&
              Array.isArray(cart.itemAuction) &&
              cart.itemAuction.length > 0 ? (
                <div>
                  {cart.itemAuction.map((item: any, index: number) => (
                    <div
                      key={item?.id || index}
                    ...
                    >
                      ...
                      <div className="flex flex-col rounded-lg bg-white sm:flex-row">
                        ...
                        <div className="flex w-full flex-col px-4 py-4">
                          ...
                          <p className="mt-auto text-lg font-bold">
                            {item?.auctionWiner?.bidPrice?.toLocaleString(
                              "vi-VN",
                              {
                                style: "currency",
                                currency: "VND",
